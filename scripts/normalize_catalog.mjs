...
function parseSpec(rawLine) {
  const line = prettifyDimensions(stripBullet(rawLine).replace(/[;.]+$/, ''))
  ...
  const label = collapseSpaces(line.slice(0, separator))
  const value = collapseSpaces(line.slice(separator + 1)).replace(/[;.,]+$/, '')
