...
Каталог сайта rutestin.com ведётся в WooCommerce, и позиции берутся оттуда
штатным экспортом, а не парсингом страниц.
...
Импортёр пропускает черновики, вариации товара и позиции, чья категория не
сопоставлена разделу сайта, — и печатает список пропущенных, чтобы это не
происходило молча. Соответствие «категория WooCommerce → раздел витрины»
задаётся в `CATEGORY_BY_NAME` внутри скрипта; вложенные категории вида
«Оборудование > Битумные эмульсии» разбираются по последнему уровню.

Импорт перезаписывает `catalog.generated.json` целиком и присваивает новые
порядковые `id`, поэтому правки в `catalog.overrides.json` привязаны не к ним,
а к `wpId` — идентификатору записи в WooCommerce, который между выгрузками не
меняется. Ключ выглядит как `wp-1009`.

Если после импорта какой-то ключ ни с чем не совпал (позицию удалили или
перенесли в другой магазин), `npm run catalog:build` печатает список таких
ключей — правка не применяется молча.
