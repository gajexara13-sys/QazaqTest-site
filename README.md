...
`id` по порядку. Правки в `catalog.overrides.json` привязаны к этим `id`,
поэтому после импорта с изменившимся составом их нужно сверить — иначе
написанные тексты уедут не на те карточки.
